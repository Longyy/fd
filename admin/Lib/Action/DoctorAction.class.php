...
class DoctorAction extends BaseAction
{
    function index()
    {
        ...
        $sql = 'SELECT t1.*,t3.`name` AS center_name,t4.sale_count 
FROM `cms_doctor` t1 
LEFT JOIN `cms_medical_center_doctor` t2 ON t1.`id`=t2.`doctor_id`
LEFT JOIN `cms_medical_center` t3 ON t2.`medical_center_id`=t3.`id`
LEFT JOIN (
  select doctor_id,count(*) as sale_count 
  from cms_consultation 
  group by doctor_id) t4 
on t1.id=t4.doctor_id limit ' . $p->firstRow . ',' . $p->listRows;
//        $doctor = $doctor->limit($p->firstRow . ',' . $p->listRows)->select();
        $doctor = $doctor->query($sql);
        $page   = $p->show();
        $this->assign('page', $page);
        $big_menu       = array('javascript:window.top.art.dialog({id:\'add\',iframe:\'?m=Doctor&a=add\', title:\'添加医生\', width:\'600\', height:\'350\', lock:true}, function(){var d = window.top.art.dialog({id:\'add\'}).data.iframe;var form = d.document.getElementById(\'dosubmit\');form.click();return false;}, function(){window.top.art.dialog({id:\'add\'}).close()});void(0);', '添加医生');
...
//        $doctor = $doctor->limit($p->firstRow . ',' . $p->listRows)->select();
        ...
        $page       = $p->show();
        $this->assign('page', $page);
        $big_menu       = array('javascript:window.top.art.dialog({id:\'add\',iframe:\'?m=Doctor&a=addDoctorWork\', title:\'添加医生门诊时间\', width:\'600\', height:\'350\', lock:true}, function(){var d = window.top.art.dialog({id:\'add\'}).data.iframe;var form = d.document.getElementById(\'dosubmit\');form.click();return false;}, function(){window.top.art.dialog({id:\'add\'}).close()});void(0);', '添加');
        $this->big_menu = $big_menu;
//dump($doctorWork);
    ...
    function  addDoctorWork()
    {
        if (isset($_POST['dosubmit'])) {
            ...
            if(!empty($workDates)) {
                $dayWork = explode(',', $workDates);
                foreach($dayWork as $day) {
                    $parts = explode('|', $day);
                    if(count($parts) != 3) {
                        continue;
                    }
                    $date = explode(':', $parts[0])[1];
                    $am = explode(':', $parts[1])[1];
                    $pm = explode(':', $parts[2])[1];
//                    dump($date);
//                    dump($am);
//                    dump($date);

                    $doctorWork = M('doctor_work_time');
                    if (!$doctorWork->where(array('doctor_id' => $_POST['doctor_id'], 'work_date' => strtotime($date)))->find()) {
                        $data = array(
