...
// | Shirazsoft APPCMS
// +----------------------------------------------------------------------
// | Copyright (c) 2010 http://shiraz-soft.com All rights reserved.
// +----------------------------------------------------------------------
...
class TagLibTpl extends TagLib{
    // 标签定义
     ...
     * 格式： 同一个页面第一次调用<wego:calendar name="star_time">{$value}</wego:calendar>
     * 同一个页面第二次调用<wego:calendar name="star_time" more="true">{$value}</wego:calendar>
     +----------------------------------------------------------
     * @access public
     +----------------------------------------------------------
     ...
     * @return string|void
     +----------------------------------------------------------
      * */
     public function _calendar($attr,$content){     	
     	$tag        =	$this->parseXmlAttr($attr,'calendar');      	   
        $name   	=	$tag['name'];
        $ifformat	=	!empty($tag['ifformat'])?$tag['ifformat']:'%Y-%m-%d %H:%M:00';
        $showstime	=	!empty($tag['showstime'])?$tag['showstime']: 'true';
        $timeformat =	!empty($tag['timeformat'])?$tag['timeformat'] :'24';
        $more=!empty($tag['more'])?$tag['more'] :'0';
        if($more=='true'){
	       $link='';
        }
        else{
        	 $link='<link rel="stylesheet" type="text/css" href="__ROOT__/statics/js/calendar/calendar-blue.css"/>
			<script type="text/javascript" src="__ROOT__/statics/js/calendar/calendar.js"></script>';
        }
        $parseStr=''.$link.'
		<input class="date input-text" type="text" name="'.$name.'" id="'.$name.'" size="18" value="'.$content.'" />	
    ...
    /**
     +----------------------------------------------------------
     * editor标签解析 插入可视化编辑器
     * 格式： <wego:editor id="editor" name="remark" type="FCKeditor" style="" >{$vo.remark}</wego:editor>
     +----------------------------------------------------------
     * @access public
     +----------------------------------------------------------
     * @param string $attr 标签属性
    ...
    /**
     +----------------------------------------------------------
     * select标签解析
     * 格式： <wego:select options="name" selected="value" />
     +----------------------------------------------------------
     * @access public
     +----------------------------------------------------------
     * @param string $attr 标签属性
    ...
    /**
     +----------------------------------------------------------
     * checkbox标签解析
     * 格式： <wego:checkbox checkboxes="" checked="" />
     +----------------------------------------------------------
     * @access public
     +----------------------------------------------------------
     * @param string $attr 标签属性
    ...
    /**
     +----------------------------------------------------------
     * radio标签解析
     * 格式： <wego:radio radios="name" checked="value" />
     +----------------------------------------------------------
     * @access public
     +----------------------------------------------------------
     * @param string $attr 标签属性
    ...
    /**
     +----------------------------------------------------------
     * list标签解析
     * 格式： <wego:grid datasource="" show="vo" />
     *
     +----------------------------------------------------------
     * @access public
     +----------------------------------------------------------
