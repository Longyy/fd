...
(function () {
    ...
    var X = function ($, C, _) {
        ...
        var A = {id: $.id, title: $.title || "\u63d0\u793a", content: $.content, iframe: $.iframe, yesText: $.yesText || "\u786e\u5b9a", noText: $.noText || "\u53d6\u6d88", yesFn: $.yesFn, noFn: $.noFn, closeFn: $.noFn, width: $.width, height: $.height, menuBtn: $.menuBtn, left: $.left, top: $.top, fixed: $.fixed, style: $.style, lock: $.lock, time: $.time};
        return I($.id).int(A)
    }, d = 4, _ = 999999, O = "temp_artDialog", S = 100000, D = "\xd7", W = "Loading..", L = 0, C = {}, R, e = [], P, N, U, M = !-[1, ], J = M && ([/MSIE (\d)\.0/i.exec(navigator.userAgent)][0][1] == 6), B = function (_, $) {
        for (var B = 0, A = _.length; B < A; B++) {
            $(_[B], B)
        }
    }, Z = function (B) {
        var _ = B ? B.document.documentElement : document.documentElement, $ = B ? B.document.body : document.body, A = _ || $;
        return{width: Math.max(A.clientWidth, A.scrollWidth), height: Math.max(A.clientHeight, A.scrollHeight), left: Math.max(_.scrollLeft, $.scrollLeft), top: Math.max(_.scrollTop, $.scrollTop), winWidth: A.clientWidth, winHeight: A.clientHeight}
    }, f = function ($, _, A) {
        if ($.attachEvent) {
            $["e" + _ + A] = A;
            $[_ + A] = function () {
        ...
        var _ = G || window.event, B = this.data.box, H = this.data.moveTemp, F = Z(), I = this.security(B);
        C.box = this;
        this.zIndex();
        P = true;
        C.x = _.clientX;
        ...
        var _ = B || window.event, E = R ? C.box.data.moveTemp : C.box.data.box, G = _.clientX, F = _.clientY, A = Z(), D = parseInt(C.left - C.x + G - C.pageLeft + A.left), $ = parseInt(C.top - C.y + F - C.pageTop + A.top);
        if (D > C.maxX) {
            D = C.maxX
        }
        if ($ > C.maxY) {
        ...
        var _ = A || window.event, F = _.clientX, E = _.clientY, $ = C.width + F - C.x + B.w, D = C.height + E - C.y + B.h;
        if ($ > 0) {
        ...
        var M = G("td"), n = G("div"), X = G("div"), t = G("a");
        ...
        n.appendChild(t);
        M.appendChild(n);
        var H = G("td"), x = G("div"), z = G("div"), q = G("div");
        ...
        H.appendChild(x);
        H.appendChild(q);
        var S = G("button"), r = G("span"), _0 = G("button"), k = G("span");
        S.setAttribute("accesskey", "y");
        r.className = "ui_yes";
        _0.setAttribute("accesskey", "n");
        k.className = "ui_no";
        var o = G("td"), I = G("div"), i = G("div"), R = G("div");
        ...
        R.onmousedown = function (A) {
            var $ = h, _ = H;
            a.call(s, A, {obj: _, w: _.offsetWidth - $.offsetWidth, h: _.offsetHeight - $.offsetHeight});
            return false
        };
        ...
        var s = {data: {box: h, moveTemp: w, selectMask: Y, wrap: m}, int: function ($) {
            s.data.config = $;
            if (typeof $.id === "string") {
                m.id = $.id
            }
            ...
            if (s.data.config.fixed) {
                G = 0;
                E = C.winWidth - s.data.boxWidth;
                _ = E / 2;
                H = 0;
                B = C.winHeight - s.data.boxHeight;
            ...
            if (I && I.getBoundingClientRect) {
                var _ = s.data.boxWidth / 2 - I.offsetWidth / 2, H = I.offsetHeight, F = I.getBoundingClientRect().left, D = I.getBoundingClientRect().top;
                if (_ > F) {
                    _ = 0
                }
            ...
            B(["ui_fixed", "ui_loading", "ui_focus", "ui_iframe"], function ($, _) {
                b(m, $)
            });
            ...
            _++;
            h.style.zIndex = E.style.zIndex = m.style.zIndex = _;
            w.style.zIndex = _ + 1;
            if (N) {
                b(N, "ui_focus")
            }
            ...
            if (m.className.indexOf("ui_lock") > -1) {
                s.alpha(E, 1, function () {
                    ...
                    T(document, "keydown", s.data.lockKey);
                    L--
                })
            }
            return s
    ...
    f(window, "load", function () {
        if (!N) {
            artDialog({id: O, style: "confirm alert error succeed", time: 10, lock: false}, function () {
            }, function () {
            })
        }
    });
