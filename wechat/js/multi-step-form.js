...
// 			and therefore the liability falls on the person who has taken it upon himself to use this script
	...
	multi_step_form = function(defaults)
	{
		...
		submit_button = '<div class="submit-data" onclick="submit()">提交</div>'; // Initialize the submit button
	...
	function ProgressBar(){
		...
		$('.progress-text').html(progressWidth+'% 完成'); // set the percentage completed so far
	}
...
//			});
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    $(section_container).html(data);
                    if(data.st){
                        $(section_container).html(data.msg);
                    }
                    else{
                        $(section_container).html(data.msg);
                    }
