...
where t1.handle_type in (1,2,3,5,6) and t1.doctor_id=$doctorId limit 0,20";
$patientResult = mysqli_query($connection, $sql) or die(mysqli_error($connection));

$sql = "SELECT * FROM `cms_order` WHERE d_id=$doctorId and status=1  and type=1 limit 0,20";
$orderResult = mysqli_query($connection, $sql) or die(mysqli_error($connection));

$sql = "SELECT p.*,c.name,c.phone FROM `cms_pic` as p left join `cms_consultation` as c on p.uid=c.id where c.doctor_id=$doctorId and p.status=1 limit 0,20";
...
<div class="con">
    ...
    <div class="ect-padding text-center">
        ...
        <a href="" class="btn btn-info ect-colorg ect-bg-grey ect-padding-lr ect-margin-lr">扫一扫</a>
    ...
    <section class="pub-tab">
        <ul class="nav nav-tabs text-center">
            <li class="col-xs-4 " onclick="jump('after')"><a data-toggle="tab" role="tab" href="#one">待处理</a></li>
            <li class="col-xs-4 cur" onclick="jump('before')"><a data-toggle="tab" role="tab" href="#two">已处理</a></li>
            <li class="col-xs-4" style="border:solid 0px red;width:100px;"><a data-toggle="tab" role="tab" href="#three">其他</a></li>
        </ul>
        <div class="tab_content">
            <div class="rl-txt" id="l3" style="display:block">
                <li id="daixiadan" onClick="ta()">求诊表</li></br>
            ...
            <div class="rl-txt" id="l3" style="display:block">
                <li id="daixiadan" onClick="ta()">代下单</li></br>
                <table id="daixiadan1" width="100%" style="display:table">
                    <tr id="shengming" align="center">
                        <td>姓名</td>
                        <td>电话</td>
                    ...
                    while ($row = $orderResult->fetch_assoc()) {
                        ?>
                        <tr onclick="detailt(<?php echo($row["id"])?>,<?php echo($row["handle_type"])?>)">
                            <td bgcolor="#ffffff"><?php echo($row["receive"]) ?></td>
                            <td bgcolor="#ffffff"><?php echo($row["phone"]) ?></td>
...
<script>
    ...
    var pageData = {'postType': 'consultationListBefore'};
                ...
                $.each(data.rt,function(index,value){
                    ...
                    switch (value.handle_type){
                        case '1':
                            ...
                            break;
                        case '2':
                            handle_type="治疗意见<br/>"+value.med_name+value+number+"瓶";
