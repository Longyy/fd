...
class AdvertAction extends BaseAction {
    public function index() {
        ...
        $id = isset($_GET['id']) && intval($_GET['id']) ? intval($this->_get('id')) : exit();
        //读取广告位信息
        ...
        if ($board_type_info['option']) {
            ...
            $ad['code']=stripslashes($ad['code']);
            $ad['code']=str_replace('script', 'script_str', $ad['code']);
            $ad['code']=str_replace('\'', '"', $ad['code']);                        
            if (!$ad) {
                $ad = array(
                    'id' => '0',
                    'type' => 'image',
        ...
        print_r($this->fetch(ROOT_PATH . '/data/adboard/' . $board_info['type'] . '.html'));exit;
    }
